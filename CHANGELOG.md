...
### Added
- Created `DashboardController.php` supporting Super Admin analytics queries and recent activity details (recent users, recent equipment, and recent bookings).
- Redesigned the primary Dashboard page (`Dashboard.tsx`) using custom `shadcn/ui` components (featuring stats grids, visual status bars, tabbed recent activities view, and a dynamic greeting banner).
